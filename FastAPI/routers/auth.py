...
async def register(sns_type: SnsType, reg_info: UserRegister):
    """
    `회원가입 API`\n
    :param sns_type:
    :param reg_info:
    ...
    if sns_type == SnsType.email:
        is_exist = await userService.is_email_exist(reg_info.email)
        ...
        regInfo = jsonable_encoder(reg_info)        
        regInfo['pw'] = hash_pw
        new_user = await userService.addUser(regInfo)
